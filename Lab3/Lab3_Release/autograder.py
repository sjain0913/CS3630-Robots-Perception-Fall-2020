...
def run_test_case(Robot_init_pose, Dh_circular, Robot_speed):
    ...
    particlefilter = ParticleFilter(particles, robbie, Dh_circular, Robot_speed, grid)
    ...
    if steps_built_track < 50:
        ...
        score1 = Steps_build_tracking-steps_built_track
    else:
        score1 = 0
...
class ParticleFilter:
    ...
    def __init__(self, particles, robbie, dh, speed, grid):
    ...
    def update(self):
        # ---------- Move Robot ----------
        odom = add_odometry_noise(move_robot_circular(self.robbie, self.dh, self.speed, self.grid), \
...
if __name__ == "__main__":
    ...
    score_with_noise = (5.0/45.0) * (score_with_noise[0]+score_with_noise[1])

    # if score_with_noise > 9:
    #     score_with_noise = 10

    score = score_without_noise #+ score_with_noise
    print("\nScore for finding global optimal estimation", score_without_noise[0])
    print("\nPose Estimation Percentage Score", score_without_noise[1])
    print("\nscore without noise =", score_without_noise[0] + score_without_noise[1])
    print("\nscore with noise =", score_with_noise)
    print("\nTotal Score =", math.ceil(score_without_noise[0]) + math.ceil(score_without_noise[1])  + math.ceil(score_with_noise))
    # stress_test()
